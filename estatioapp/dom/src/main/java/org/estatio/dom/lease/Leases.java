...
public class Leases extends EstatioDomainService<Lease> {
    ...
    public Lease newLease(
            ...
            final @ParameterLayout(named = "Duration", describedAs = "Duration in a text format. Example 6y5m2d") @Parameter(optional = Optionality.TRUE) String duration,
            final @ParameterLayout(named = "End Date", describedAs = "Can be omitted when duration is filled in") @Parameter(optional = Optionality.TRUE) LocalDate endDate,
            final @ParameterLayout(named = "Landlord") @Parameter(optional = Optionality.TRUE) Party landlord,
            final @ParameterLayout(named = "Tentant") @Parameter(optional = Optionality.TRUE) Party tenant
            // CHECKSTYLE:ON
            ) {
